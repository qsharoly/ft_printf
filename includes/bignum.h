/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   bignum.h                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: qsharoly <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/05/12 04:19:11 by qsharoly          #+#    #+#             */
/*   Updated: 2021/02/18 18:54:34 by debby            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef BIGNUM_H
# define BIGNUM_H

# include <limits.h>

/*
** Use base 1e+8 so that result of multiplication of two `t_digit`s
** will always fit into a `long long` aka `t_digit_mul`.
** This allows for simpler math. Also: easier decimal printing.
**
*/

# define BIG_N_DIGITS 30
# define BIG_BASE 100000000
# define BIG_BASE_CHARS 8
# define BIG_TO_STR_BUFSIZE (BIG_BASE_CHARS * BIG_N_DIGITS + 1)

typedef unsigned long	t_digit;
typedef long long		t_digit_tmp;

typedef	struct			s_big
{
	t_digit	val[BIG_N_DIGITS];
}						t_big;

t_big					big_zero(void);
t_big					big_from_number(unsigned long n);
t_big					big_add(t_big a, t_big b);
t_big					big_mul(t_big a, t_big b);
t_big					big_raise(t_digit n, t_digit power);
char					*big_str(char buf[BIG_TO_STR_BUFSIZE], t_big a);

#endif
