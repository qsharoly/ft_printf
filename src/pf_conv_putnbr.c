...
/*   Updated: 2022/10/22 13:59:45 by debby            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pf_putnbr(t_stream *out, t_sv value, t_sv prefix, const t_fmt *fmt)
{
	...
	if (fmt->align_right_by_leading_zeros && fmt->align == Align_right && !fmt->has_precision)
	{
	...
	put_repeat(' ', (fmt->align == Align_right) * pad_length, out);
	put_sv(prefix, out);
	put_repeat('0', zero_fill, out);
	put_sv(value, out);
	put_repeat(' ', (fmt->align == Align_left) * pad_length, out);
}
